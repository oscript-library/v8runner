...
Функция ПолучитьВерсиюРасширенияИзХранилища(Знач ИмяРасширения, Знач СтрокаСоединения, Знач ПользовательХранилища, Знач ПарольХранилища, Знач НомерВерсии = Неопределено) Экспорт
...
Процедура ПолучитьОтчетПоВерсиямРасширенияИзХранилища(Знач ИмяРасширения,
...
Процедура СоздатьФайловоеХранилищеРасширения(
	Знач ИмяРасширения,
	Знач ПапкаХранилища, 
	Знач ПользовательХранилища, 
	Знач ПарольХранилища = "", 
...
//
// Параметры:
//  ИмяРасширения    	  - Строка - Наименование расширения конфигурации
//  СтрокаСоединения 	  - Строка - Строка соединения с хранилищем конфигурации
//  ПользовательХранилища - строка - Пользователь для подключения к хранилищю конфигурации
//  ПарольХранилища 	  - Строка - Пароль пользователь для подключения к хранилищю конфигурации
//  ПутьКФайлуСоСпискомОбъектов - Строка - Строка путь к файлу xml c содержанием в формате http://its.1c.ru/db/v839doc#bookmark:adm:TI000000712
...
Процедура ЗахватитьОбъектыРасширенияВХранилище(
	Знач ИмяРасширения,
	Знач СтрокаСоединения, 
	Знач ПользовательХранилища,
	Знач ПарольХранилища = "",
...
КонецПроцедуры // ЗахватитьОбъектыРасширенияВХранилище()
...
// Отмена захват объектов для редактирования в хранилище расширения конфигурации
//
// Параметры:
//  ИмяРасширения         - Строка - Наименование расширения конфигурации
//  СтрокаСоединения 	  - Строка - Строка соединения с хранилищем конфигурации
...
//									 Если опция не указана, то при наличии локально измененных объектов операция будет отменена и будет выдана ошибка.
//
Процедура ОтменитьЗахватОбъектовРасширенияВХранилище(Знач ИмяРасширения,
	Знач СтрокаСоединения, 
	Знач ПользовательХранилища,
	Знач ПарольХранилища = "",
	Знач СписокОбъектов = Неопределено,
...
КонецПроцедуры // ОтменитьЗахватОбъектовРасширенияВХранилище()
...
// Помещение изменений объектов в хранилище расширения конфигурации 
//
// Параметры:
//  ИмяРасширения         - Строка - Наименование расширения конфигурации
//  СтрокаСоединения 	  - Строка - Строка соединения с хранилищем конфигурации
...
//                                   Если опция не указана, при обнаружении ссылок на удаленные объекты будет выдана ошибка.
//
Процедура ПоместитьИзмененияОбъектовРасширенияВХранилище(Знач ИмяРасширения,
	Знач СтрокаСоединения, 
	Знач ПользовательХранилища,
	Знач ПарольХранилища = "",
	Знач СписокОбъектов = Неопределено,
...
КонецПроцедуры // ПоместитьИзмененияОбъектовРасширенияВХранилище()
...
// Выполняет подключение ранее неподключенного расширения к хранилищу расширения конфигурации.
//
// Параметры:
//  ИмяРасширения         - Строка - Наименование расширения конфигурации
//  СтрокаСоединения 	  - Строка - Строка соединения с хранилищем конфигурации
...
//									 Если конфигурация непустая, данный ключ подтверждает замену конфигурации на конфигурацию из хранилища.
//
Процедура ПодключитьРасширениеКХранилищу(Знач ИмяРасширения,
	Знач СтрокаСоединения, 
	Знач ПользовательХранилища,
	Знач ПарольХранилища = "",
	Знач ИгнорироватьНаличиеПодключеннойБД = Ложь,
...
// Параметры:
//  ИмяРасширения         - Строка - Наименование расширения конфигурации
Процедура ОтключитьРасширениеОтХранилища(Знач ИмяРасширения) Экспорт
	Параметры = СтандартныеПараметрыЗапускаКонфигуратора();
...
// Добавление пользователя хранилища расширения конфигурации. 
//	Пользователь, от имени которого выполняется подключение к хранилищу, должен обладать административными правами. 
//	Если пользователь с указанным именем существует, то пользователь добавлен не будет.
// Параметры:
//  ИмяРасширения         - Строка - Наименование расширения конфигурации
...
//								       Если обнаружен удаленный пользователь с таким же именем, он будет восстановлен.
//  		 
Процедура ДобавитьПользователяВХранилищеРасширения(Знач ИмяРасширения,
	Знач СтрокаСоединения, 
	Знач ПользовательХранилища, 
	Знач ПарольХранилища,
	Знач НовыйПользователь,
...
// Копирование пользователей из хранилища расширения конфигурации. Копирование удаленных пользователей не выполняется. 
//   Если пользователь с указанным именем существует, то пользователь не будет добавлен.
// 
// Параметры:
//  ИмяРасширения         - Строка - Наименование расширения конфигурации
...
//   ВосстановитьУдаленного - Булево - флаг небходимости востановления удаленных пользователей
//		 
Процедура КопироватьПользователейИзХранилищаРасширения(Знач ИмяРасширения,
	Знач СтрокаСоединения, 
	Знач ПользовательХранилища, 
	Знач ПарольХранилища,
	Знач СтрокаСоединенияХранилищаКопии,
...
Функция ПолучитьПутьКВерсииПлатформы(Знач ВерсияПлатформы) Экспорт
	...
	Если ЭтоWindows = Истина Тогда
		...
		Для Каждого ВозможныйПуть Из КаталогиУстановкиПлатформы Цикл
			...
			Иначе
				Для каждого Подкаталог1С Из МассивФайлов Цикл
					...
					Иначе
						Лог.Отладка("Добавляем путь к файлу запуска учебной версии платформы.");
						Файл1cv8t = Новый Файл(ОбъединитьПути(Подкаталог1С.ПолноеИмя, "bin", "1cv8t.exe"));			
						
						Если Файл1cv8t.Существует() Тогда
							ОписаниеКаталога = Новый Структура("Версия, ФайлКлиента1С", Подкаталог1С.Имя, Файл1cv8t);
							МассивКаталогов1С.Добавить(ОписаниеКаталога);
							Лог.Отладка("Найдена учебная версия платформы 1С %1, некоторые команды будут недоступны(см. ограничения учебной версии платформы 1С).", Подкаталог1С.Имя);
						Иначе
		...
		Если МассивКаталоговВерсий.Количество() > 0 Тогда
			...
			Лог.Отладка("Устанавливаем признак использования учебной версии платформы.");
			Если НЕ (НужныйФайлПлатформы.Имя = "") Тогда				
				мУчебнаяВерсияПлатформы = СтрНайти(НужныйФайлПлатформы.Имя, "1cv8t")<>0;
			КонецЕсли;
		КонецЕсли;
