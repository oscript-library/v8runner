...
Функция СформироватьСтрокуСоединения(ПараметрыСтрокиСоединения) Экспорт
	...
	Если ПараметрыСтрокиСоединения.Свойство("КлючРазрешенияЗапуска") Тогда
		СтрокаСоединения = СтрШаблон("%1 /UC""%2""", СтрокаСоединения, ПараметрыСтрокиСоединения.КлючРазрешенияЗапуска);
	КонецЕсли;
