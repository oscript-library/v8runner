...
Процедура ЗагрузитьКонфигурациюИзФайлов(Знач КаталогЗагрузки, 
	...
	Если ИспользуемаяВерсияПлатформыСтаршеИлиРавна("8.3.10") Тогда
		...
		Если ЗначениеЗаполнено(ПутьКСпискуФайловЗагрузки) Тогда
			...
			ПутьКСпискуФайловЗагрузки_Обработанный = ПолучитьОбработанныйФайлСписка(ПутьКСпискуФайловЗагрузки);
...
//Функция удаляет строки модулей форм из списка файлов, оставляя только файлы .xml форм
...
//
// Возвращаемое значение:
//	Строка - Путь к новому файлу в кодировке UTF-8 списка для опции -listFile команды /LoadConfigFromFiles
Функция ПолучитьОбработанныйФайлСписка(ПутьКСпискуФайловЗагрузки) Экспорт
	Кодировка = "UTF-8";
	СтрокаПоиска = "\Ext\Form\Module.bsl";
	СтрокаЗамены = ".xml";
	МассивПоискаДублей = Новый Массив;
	ОбработанныйФайл = ПолучитьИмяВременногоФайла(".txt");
	Запись = Новый ЗаписьТекста(ОбработанныйФайл, Кодировка);
