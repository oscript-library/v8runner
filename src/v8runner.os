...
// Возвращает путь каталогу с конфигурационными файлами 1С 
//
// Параметры:
//  ПутьКПлатформе1С     - Строка - Полный путь к платформе 1С, 
//                 					например, C:\Program Files\1cv8\8.3.17.1386\bin\1cv8.exe
//  ПутьКОбщимНастройкам - Булево - Если требуется получить путь к каталогу с общими натсройками - Истина.
...
//
Функция ПутьККонфигу1С(Знач ПутьКПлатформе1С = "", Знач ПутьКОбщимНастройкам = Истина) Экспорт
	
	Лог.Отладка("ПутьККонфигу1С: Путь платформы 1С <"+ПутьКПлатформе1С + ">");
	Если ПутьКПлатформе1С = "" Тогда
		ПутьКПлатформе1С = ПутьКПлатформе1С();
		Лог.Отладка("ПутьККонфигу1С: вычислили Путь платформы 1С <"+ПутьКПлатформе1С + ">");
	КонецЕсли;

	КаталогНаДиске = Новый Файл(ПутьКПлатформе1С);
	Если Не КаталогНаДиске.Существует() Тогда
		ВызватьИсключение "Не установлена платформа";
	КонецЕсли;	
	...
	Если ПутьКОбщимНастройкам Тогда
		ПутьККонфигу1С = СтрЗаменить(ПутьКПлатформе1С, "\" + ВерсияПлатформы + "\bin\1cv8.exe", "\conf");
		Лог.Отладка("ПутьККонфигу1С: получили путь к конфигу 1С (Общий) <" + ПутьККонфигу1С + ">");	
	Иначе
		ПутьККонфигу1С = СтрЗаменить(ПутьКПлатформе1С, "\1cv8.exe", "\conf");
		Лог.Отладка("ПутьККонфигу1С: получили путь к конфигу 1С (Для текущей версии) <" + ПутьККонфигу1С + ">");
	КонецЕсли;	
	Возврат ПутьККонфигу1С;
