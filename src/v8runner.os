...
Функция ПолучитьОтчетПоВерсиямИзХранилища(Знач СтрокаСоединения, 
						Знач ПользовательХранилища, 
						Знач ПарольХранилища = "", 
						Знач НомерНачальнойВерсии = 0,
						Знач НомерКонечнойВерсии = Неопределено,
						Знач ТипГруппировки = "–GroupByComment") Экспорт
	...
	ФайлРезультата = ОбъединитьПути(КаталогСборки(), "storage-report.mxl");

	Параметры.Добавить("/ConfigurationRepositoryReport """+ФайлРезультата + """");
	...
	Если Не ПустаяСтрока(ТипГруппировки) Тогда
