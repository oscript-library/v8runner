...
Процедура СоздатьСервернуюБазу(ПараметрыБазы1С,
	...
	Сервер1С				= ПараметрыБазы1С.Сервер1С;
	ИмяИБ					= ПараметрыБазы1С.ИмяИБ;

	Лог.Отладка("Создание серверной ИБ ""%1\%2""", Сервер1С, ИмяИБ);
	
	РазрешитьРегЗадания		= Ложь;
	Если ПараметрыБазы1С.Свойство("РазрешитьРегЗадания") Тогда
		РазрешитьРегЗадания	= ПараметрыБазы1С.РазрешитьРегЗадания;
	КонецЕсли;

	ТипСУБД				= "MSSQLServer";
	Если ПараметрыСУБД.Свойство("ТипСУБД") Тогда
		ТипСУБД			= ПараметрыСУБД.ТипСУБД;
	КонецЕсли;
	СерверСУБД			= ПараметрыСУБД.СерверСУБД;
	ПользовательСУБД	= ПараметрыСУБД.ПользовательСУБД;
	ПарольСУБД			= "MSSQLServer";
	Если ПараметрыСУБД.Свойство("ПарольСУБД") Тогда
	...
	СмещениеДат			= "2000";
	Если ПараметрыСУБД.Свойство("СмещениеДат") Тогда
		СмещениеДат		= ПараметрыСУБД.СмещениеДат;
	КонецЕсли;
	СоздаватьБД			= Ложь;
	Если ПараметрыСУБД.Свойство("СоздаватьБД") Тогда
		СоздаватьБД		= ПараметрыСУБД.СоздаватьБД;
	КонецЕсли;

	АдминистраторКластера		= Ложь;
	Если АвторизацияВКластере.Свойство("Имя") Тогда
		АдминистраторКластера	= АвторизацияВКластере.Имя;
	КонецЕсли;
	ПарольКластера				= Ложь;
	Если АвторизацияВКластере.Свойство("Пароль") Тогда
		ПарольКластера			= АвторизацияВКластере.Пароль;
	КонецЕсли;
