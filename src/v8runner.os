...
Процедура ЗагрузитьКонфигурациюИзФайлов(Знач КаталогЗагрузки, 
	...
	ПутьКСпискуФайловЗагрузки_Обработанный = "";
	...
	Если ИспользуемаяВерсияПлатформыСтаршеИлиРавна("8.3.10") Тогда
		...
		Если ЗначениеЗаполнено(ПутьКСпискуФайловЗагрузки) Тогда
			...
			ПутьКСпискуФайловЗагрузки_Обработанный = ВременныеФайлы.НовоеИмяФайла("txt");
			ПолучитьОбработанныйФайлСписка(ПутьКСпискуФайловЗагрузки, ПутьКСпискуФайловЗагрузки_Обработанный);
			
			Параметры.Добавить(СтрШаблон("-listFile %1", ОбернутьВКавычки(ПутьКСпискуФайловЗагрузки_Обработанный)));
			Параметры.Добавить(СтрШаблон("-format %1", ФорматВыгрузки));
	...
	Если НЕ ПустаяСтрока(ПутьКСпискуФайловЗагрузки_Обработанный) Тогда
		ВременныеФайлы.УдалитьФайл(ПутьКСпискуФайловЗагрузки_Обработанный);
	КонецЕсли;
...
//Процедура удаляет строки модулей форм из списка файлов, оставляя только файлы .xml форм
//Платформа выдает ошибку "Неизвестный объект метаданных Document.<ИмяДокумента>.Form.ФормаДокумента.Ext", если передать строку "*\Ext\Form\Module.bsl"
//Такие строки следует заменять на "*.xml", команда /LoadConfigFromFiles вместе с .xml файлом сама затянет модуль формы
//
// Параметры:
//	ПутьКСпискуФайловЗагрузки - Строка - Путь к файлу списка в кодировке UTF-8 для опции -listFile команды /LoadConfigFromFiles
//	ПутьКСпискуФайловЗагрузки_Обработанный - Строка - Путь к файлу списка в кодировке UTF-8 для опции -listFile команды /LoadConfigFromFiles
//
Процедура ПолучитьОбработанныйФайлСписка(ПутьКСпискуФайловЗагрузки, ПутьКСпискуФайловЗагрузки_Обработанный) Экспорт
	Кодировка = "UTF-8";
	СтрокаПоиска = "\Ext\Form\Module.bsl";
	СтрокаЗамены = ".xml";
	МассивПоискаДублей = Новый Массив;
	Чтение = Новый ЧтениеТекста(ПутьКСпискуФайловЗагрузки, Кодировка);
	Запись = Новый ЗаписьТекста(ПутьКСпискуФайловЗагрузки_Обработанный, Кодировка);
	ТекущаяСтрока = Чтение.ПрочитатьСтроку();
	Пока ТекущаяСтрока <> Неопределено Цикл
		ТекущаяСтрока = СтрЗаменить(ТекущаяСтрока, СтрокаПоиска, СтрокаЗамены);
		Если МассивПоискаДублей.Найти(ТекущаяСтрока) = Неопределено Тогда
			МассивПоискаДублей.Добавить(ТекущаяСтрока);
			Запись.ЗаписатьСтроку(ТекущаяСтрока);
		КонецЕсли;
		ТекущаяСтрока = Чтение.ПрочитатьСтроку();
	КонецЦикла;
	Чтение.Закрыть();
