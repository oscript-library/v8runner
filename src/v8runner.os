...
Функция СоздатьФайлыПоставкиВерсииИзХранилища(знач КаталогПоставки, 
											Знач НомерВерсииПоставки, 
											Знач СтрокаСоединения, 
											Знач ПользовательХранилища, 
											Знач ПарольХранилища = "", 
											знач ПолнаяПоставка = Истина) Экспорт

	//ПолучитьИзмененияКонфигурацииБазыДанныхИзХранилища(СтрокаСоединения,  ПользовательХранилища, ПарольХранилища);

	ОбновитьКонфигурациюБазыДанныхИзХранилища(СтрокаСоединения,  ПользовательХранилища, ПарольХранилища);

	ОбеспечитьКаталог(КаталогПоставки);
	
	Параметры = СтандартныеПараметрыЗапускаКонфигуратора();
		
	ФайлПолнойПоставки = ОбъединитьПути(КаталогПоставки, НомерВерсииПоставки+".cf");
	
	Параметры.Добавить("/CreateDistributionFiles -cffile """+ФайлПолнойПоставки+"""");

	ВыполнитьКоманду(Параметры);

	Возврат ФайлПолнойПоставки;

КонецФункции

Процедура СоздатьХранилищеКонфигурации(знач ФайлКонфигурации, 
											Знач ПапкаХранилища, 
											Знач ПользовательХранилища, 
											Знач ПарольХранилища = "", 
...
Функция ХранилищеКонфигурацииСуществует(ПапкаХранилища) Экспорт
...
Процедура ПолучитьИзмененияКонфигурацииБазыДанныхИзХранилища(СтрокаСоединения, ПользовательХранилища, ПарольХранилища="") Экспорт
