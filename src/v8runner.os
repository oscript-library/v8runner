...
Функция ПолучитьПутьКВерсииПлатформы(Знач ВерсияПлатформы) Экспорт
	...
	Если ЭтоWindows = Истина Тогда
			...
			Иначе
				Для каждого Подкаталог1С Из МассивФайлов Цикл
					...
					Если Файл1cv8.Существует() Тогда
						ОписаниеКаталога = Новый Структура("Имя, ФайлКлиента1С", Подкаталог1С.Имя, Файл1cv8);
		...
		Для Каждого ОписаниеКаталога Из МассивКаталогов1С Цикл
			правыйСимвол = Прав(ОписаниеКаталога.Имя, 1);
		...
		Если МассивКаталоговВерсий.Количество() > 0 Тогда
			...
			Для Сч = 1 По МассивКаталоговВерсий.ВГраница() Цикл
				Если СтроковыеФункции.СравнитьВерсии(МассивКаталоговВерсий[Сч].Имя, ОписаниеМаксВерсии.Имя) > 0 Тогда
...
Функция ИспользуемаяВерсияПлатформыСтаршеИлиРавна(Знач ТребуемаяВерсияПлатформы) Экспорт
	...
	ЭталоннаяВерсия = ТребуемаяВерсияПлатформы;
	
	ВерсияБольше = СтроковыеФункции.СравнитьВерсии(СравниваемаяВерсия, ЭталоннаяВерсия) > 0;
	ВерсияРавна = СтроковыеФункции.СравнитьВерсии(СравниваемаяВерсия, ЭталоннаяВерсия) = 0;

	Лог.Отладка(СтрШаблон("Сравниваю версию: %1 с эталоном: %2",СравниваемаяВерсия, ЭталоннаяВерсия));

	Возврат ВерсияРавна ИЛИ ВерсияБольше;
