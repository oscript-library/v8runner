...
Функция ПолучитьПутьКВерсииПлатформы(Знач ВерсияПлатформы) Экспорт
	...
	Иначе
		...
		КаталогУстановкиi386 = Новый Файл(ОбъединитьПути(КорневойПуть1С, "i386"));
		КаталогУстановкиx86_64 = Новый Файл(ОбъединитьПути(КорневойПуть1С, "x86_64"));
		
		Если КаталогУстановкиi386.Существует() Тогда
			КаталогУстановки = КаталогУстановкиi386;
		ИначеЕсли КаталогУстановкиx86_64.Существует() Тогда
			КаталогУстановки = КаталогУстановкиx86_64;
		Иначе
			КаталогВерсииUnix = ОбъединитьПути("/opt", "1cv8");
			НужныйФайлПлатформы = ПоискВерсииВКаталогеВерсий(КаталогВерсииUnix, ВерсияПлатформы);
		КонецЕсли;
...
Функция ПоискВерсииВКаталогеВерсий(Знач КаталогВерсийПлатформы, Знач ВерсияПлатформы)
	Лог.Отладка("Выполняю попытку поиска версии в каталоге <%1>", КаталогВерсийПлатформы);
	
	МассивФайлов = НайтиФайлы(КаталогВерсийПлатформы, ВерсияПлатформы+"*");
	Если МассивФайлов.Количество() = 0 Тогда
		Лог.Отладка("Не найдено ни одного каталога с версией в %1", КаталогВерсийПлатформы);
		Возврат Неопределено;
	КонецЕсли;

	МассивКаталогов1С = Новый Массив();

	ИмяФайла1СПриложение = ?(ЭтоWindows, ОбъединитьПути("bin","1cv8.exe"), "1cv8");
	ИмяФайла1СПриложениеУчебнойВерсии = ?(ЭтоWindows, ОбъединитьПути("bin","1cv8t.exe"), "1cv8t");

	Для каждого Подкаталог1С Из МассивФайлов Цикл
		Файл1cv8 = Новый Файл(ОбъединитьПути(Подкаталог1С.ПолноеИмя, ИмяФайла1СПриложение));
		Если Файл1cv8.Существует() Тогда
			ОписаниеКаталога = Новый Структура("Версия, ФайлКлиента1С", Подкаталог1С.Имя, Файл1cv8);
			МассивКаталогов1С.Добавить(ОписаниеКаталога);
			Лог.Отладка("Нашел платформу 1С %1", Подкаталог1С.Имя);
		Иначе
			Лог.Отладка("Добавляем путь к файлу запуска учебной версии платформы.");
			Файл1cv8t = Новый Файл(ОбъединитьПути(Подкаталог1С.ПолноеИмя, ИмяФайла1СПриложениеУчебнойВерсии));			
