...
Процедура ВыгрузитьИзмененияКонфигурацииВФайл(Знач КаталогВыгрузки, Знач ПутьКФайлуИзменений, Знач ПутьКФайлуВерсийДляСравнения = "", Знач ФорматВыгрузки = "") Экспорт
	...
	Если НЕ ИспользуемаяВерсияПлатформыСтаршеИлиРавна("8.3.10") Тогда
		ВызватьИсключение "Данная процедура не поддерживается выбранной версией платформы";
	КонецЕсли;
...
Функция ИспользуемаяВерсияПлатформыСтаршеИлиРавна(Знач ТребуемаяВерсияПлатформы) Экспорт
	...
	МассивЗначениеТребуемойВерсии = СтрРазделить(ТребуемаяВерсияПлатформы, ".");
	ДлинаТребуемойВерсии = МассивЗначениеТребуемойВерсии.Количество();
	
	МассивЗначениеТекущейВерсии = СтрРазделить(мВерсияПлатформы, ".");
	ДлинаТекущейВерсии = МассивЗначениеТекущейВерсии.Количество();
	
	СравниваемаяВерсия = мВерсияПлатформы;

	Если ДлинаТекущейВерсии > ДлинаТребуемойВерсии Тогда
		
		ЭлементовДляУдаления =  ДлинаТекущейВерсии - ДлинаТребуемойВерсии;
	
		Для ИИ = 1 По ЭлементовДляУдаления Цикл

			МассивЗначениеТекущейВерсии.Удалить(МассивЗначениеТекущейВерсии.ВГраница());

		КонецЦикла;
 		 
		СравниваемаяВерсия = СобратьВерсииИзМассива(МассивЗначениеТекущейВерсии);

	КонецЕсли;
	
	СравниваемаяВерсия = ПровестиВерсиюКРазрядности(СравниваемаяВерсия);
	ЭталоннаяВерсия = ПровестиВерсиюКРазрядности(ТребуемаяВерсияПлатформы);
	
	Лог.Отладка(СтрШаблон("Сравниваю версию: %1 с эталоном: %2",СравниваемаяВерсия, ЭталоннаяВерсия));

	Возврат ВерсияСтаршеИлиРавна(СравниваемаяВерсия, ЭталоннаяВерсия);

КонецФункции

Функция ПровестиВерсиюКРазрядности(Знач Версия, Знач НужнаяРазрядность = 4)
	
	МассивЗначениеВерсии = СтрРазделить(Версия, ".");

	ТекущаяРазрядность = МассивЗначениеВерсии.Количество();
	
	Пока ТекущаяРазрядность <= НужнаяРазрядность Цикл
		Если ТекущаяРазрядность = НужнаяРазрядность Тогда
			Прервать;
		КонецЕсли;

		МассивЗначениеВерсии.Добавить("0");
		
		ТекущаяРазрядность = МассивЗначениеВерсии.Количество();

	КонецЦикла;
	
	НоваяВерсия = СобратьВерсииИзМассива(МассивЗначениеВерсии);

	Возврат НоваяВерсия;

КонецФункции

Функция СобратьВерсииИзМассива(Знач МассивЗначениеВерсии)
	
	Возврат СтрСоединить(МассивЗначениеВерсии, ".");

КонецФункции // СобратьВерсииДляСравнения()

Функция ВерсияСтаршеИлиРавна(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	
	Возврат (ВерсияБольше(СтрокаВерсии1, СтрокаВерсии2) ИЛИ ВерсииРавны(СтрокаВерсии1, СтрокаВерсии2));

КонецФункции

// Сравнение версий на больше
//
Функция ВерсияБольше(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
	
	Возврат СравнитьВерсии(СтрокаВерсии1, СтрокаВерсии2) > 0;

КонецФункции

// Сравнение версий на равность
//
Функция ВерсииРавны(Знач СтрокаВерсии1, Знач СтрокаВерсии2)
