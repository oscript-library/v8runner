...
Процедура ТестДолжен_СоздатьХранилищеКонфигурации() Экспорт
	...
	КаталогВремененногоХранилища = ОбъединитьПути(ВременныйКаталог, "v8r_TempRepository");
...
Процедура ТестДолжен_ПроверитьСозданиеФайловПоставки() Экспорт
	...
	ФайлПоставки = ОбъединитьПути(КаталогПоставки, НомерВерсииВыпуска);
	
	УправлениеКонфигуратором.ЗагрузитьКонфигурациюИзФайла(ФайлКонфигурации, Истина);
	
	юТест.ПроверитьЛожь(ФайлСуществует(ФайлПоставки), "Файла поставки конфигурации не должно быть в катологе <"+КаталогПоставки+">");
	
	УправлениеКонфигуратором.СоздатьФайлыПоставки(КаталогПоставки, НомерВерсииВыпуска);
	
	юТест.ПроверитьИстину(ФайлСуществует(ФайлПоставки), "Файл посткавки конфигурации должен существовать");
